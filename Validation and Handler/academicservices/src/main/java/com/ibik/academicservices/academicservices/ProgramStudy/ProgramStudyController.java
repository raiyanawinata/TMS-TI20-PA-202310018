...
public class ProgramStudyController {
    ...
    // }
    public ResponseEntity<ResponseData<ProgramsStudy>> postPrograms(@Valid @RequestBody ProgramsStudy programsStudy, Errors errors){
...
return ResponseEntity.ok(responseData);
    ...
    public ResponseEntity<ResponseData<ProgramsStudy>>fetchProgramsStudy(){
        ...
        try {
            responseData.setResult(true);
            List<ProgramsStudy> value = (List<ProgramsStudy>)programsStudyServices.findAll();
            responseData.setData(value);
            return ResponseEntity.ok(responseData);
        } catch (Exception e) {
           responseData.setResult(false);
    ...
    //     return programsStudyServices.save(programsStudy);
    // }
    public ResponseEntity<ResponseData<ProgramsStudy>> updateProgramsStudy(@Valid @RequestBody ProgramsStudy programsStudy, Errors errors){
        ResponseData<ProgramsStudy> responseData = new ResponseData<>();
        if(errors.hasErrors()){
            for(ObjectError error : errors.getAllErrors()){
                responseData.getMessage().add(error.getDefaultMessage());
